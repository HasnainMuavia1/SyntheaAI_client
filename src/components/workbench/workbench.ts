...
// Add these imports for command execution
    ...
    startup(): IInstantiationService {
        try {
            ...
            instantiationService.invokeFunction(accessor => {
                ...
                // --------------------------------------------------------------------------------
                // CUSTOM VOICE AGENT INJECTION (ADDED)
                // --------------------------------------------------------------------------------
                
                // We listen for messages from the parent window (Next.js App)
                mainWindow.addEventListener('message', async (event) => {
                    const message = event.data;

                    if (message && message.type === 'VOICE_COMMAND') {
                        const commandText = message.payload;
                        console.log("[VS Code Internal] Received Voice Command:", commandText);

                        const commandService = accessor.get(ICommandService);
                        const editorService = accessor.get(IEditorService);

                        try {
                            // Example mappings - You will expand this with your LLM parsed actions
                            if (commandText.includes("save file")) {
                                await commandService.executeCommand('workbench.action.files.save');
                            } 
                            else if (commandText.includes("close tab")) {
                                await commandService.executeCommand('workbench.action.closeActiveEditor');
                            }
                            else if (commandText.includes("new file")) {
                                await commandService.executeCommand('workbench.action.files.newUntitledFile');
                            }
                             else if (commandText.includes("open settings")) {
                                await commandService.executeCommand('workbench.action.openSettings');
                            }
                            // Add more commands or a parser here
                            
                        } catch (err) {
                            console.error("[VS Code Internal] Failed to execute voice command", err);
                        }
                    }
                });
                // --------------------------------------------------------------------------------
